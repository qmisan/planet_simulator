...
def gravity(element1,element2):
    """
    This needs to return force vector (so that it has direction) so it must be scalar
    times unit vector from element1.location to element2.location
    """
    return (((element1.mass*element2.mass)/distance(element1.position,element2.position)**2) 
        *6.67384**(-11))*unit_vector(element2.position - element1.position)
